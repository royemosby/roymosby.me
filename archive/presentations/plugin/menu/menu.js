  ...
  (function () {
    ...
    loadResource(options.path + '/lib/jeesh.min.js', 'script', function () {
      loadResource(options.path + '/lib/bowser.min.js', 'script', function () {
        loadResource(options.path + '/menu.css', 'stylesheet', function () {
          loadResource(
            options.path + '/font-awesome-4.3.0/css/font-awesome.min.css',
            'stylesheet',
            function () {
              ...
              if (!bowser.msie || bowser.version >= 9) {
                ...
                const hideMissingTitles = options.hideMissingTitles || false
                ...
                if (typeof openSlideNumber === 'undefined')
                ...
                function visibleOffset(el) {
                  const offsetFromTop =
                    getOffset(el).top - el.offsetParent.offsetTop
                    ...
                    switch (event.keyCode) {
                      ...
                      case 85:
                        ...
                        var firstVisible =
                          itemsAbove.length > 0 &&
                          Math.abs(
                            visibleOffset(itemsAbove[itemsAbove.length - 1])
                          ) < itemsAbove[itemsAbove.length - 1].clientHeight
                            ? itemsAbove[itemsAbove.length - 1]
                            : visibleItems[0]
                        if (firstVisible) {
                          if (
                            $(firstVisible).hasClass('selected') &&
                            itemsAbove.length > 0
                          ) {
                      ...
                      case 35:
                        ...
                        var sel = $(
                          '.active-menu-panel .slide-menu-items li:last-of-type'
                        )
                        if (sel.length > 0) {
                          keepVisible(sel.addClass('selected').get(0))
                        }
                ...
                function openPanel(e) {
                  ...
                  $('.slide-menu-toolbar > li').removeClass(
                    'active-toolbar-button'
                  )
                  $('li[data-panel="' + panel + '"]').addClass(
                    'active-toolbar-button'
                  )
                  $('.slide-menu-panel').removeClass('active-menu-panel')
                  $('div[data-panel="' + panel + '"]').addClass(
                    'active-menu-panel'
                  )
                }
                ...
                function nextPanel() {
                  const next =
                    ($('.active-toolbar-button').data('button') + 1) % buttons
                  openPanel(
                    $('.toolbar-panel-button[data-button="' + next + '"]').data(
                      'panel'
                    )
                  )
                }
                ...
                function prevPanel() {
                  let next = $('.active-toolbar-button').data('button') - 1
                  if (next < 0) {
                    next = buttons - 1
                  }
                  openPanel(
                ...
                const toolbar = $(
                  '<ol class="slide-menu-toolbar"></ol>'
                ).prependTo($('.slide-menu'))
                var buttons = 0
                $(
                  '<li data-panel="Slides" data-button="' +
                    buttons++ +
                    '" class="toolbar-panel-button"><span class="slide-menu-toolbar-label">Slides</span><br/><i class="fa fa-list"></i></li>'
                )
                  .appendTo(toolbar)
                ...
                if (custom) {
                  custom.forEach(function (element, index, array) {
                    $(
                      '<li data-panel="Custom' +
                        index +
                        '" data-button="' +
                        buttons++ +
                        '" class="toolbar-panel-button"><span class="slide-menu-toolbar-label">' +
                        element.title +
                        '</span><br/>' +
                        element.icon +
                        '</i></li>'
                ...
                if (themes) {
                  $(
                    '<li data-panel="Themes" data-button="' +
                      buttons++ +
                      '" class="toolbar-panel-button"><span class="slide-menu-toolbar-label">Themes</span><br/><i class="fa fa-desktop"></i></li>'
                  )
                    .appendTo(toolbar)
                    .click(openPanel)
                }
                if (transitions) {
                  $(
                    '<li data-panel="Transitions" data-button="' +
                      buttons++ +
                      '" class="toolbar-panel-button"><span class="slide-menu-toolbar-label">Transitions</span><br/><i class="fa fa-arrows-h"></i></li>'
                  )
                    .appendTo(toolbar)
                    .click(openPanel)
                }
                ...
                function generateItem(type, section, i, h, v) {
                  ...
                  title =
                    '<span class="slide-menu-item-title">' + title + '</span>'
                  if (numbers) {
                    ...
                    title =
                      '<span class="slide-menu-item-number">' +
                      value.join('') +
                      '. </span>' +
                      title
                  }
                  ...
                  if (markers) {
                    m =
                      '<i class="fa fa-check-circle past"></i>' +
                ...
                function openItem(item, force) {
                  const h = $(item).data('slide-h')
                  const v = $(item).data('slide-v')
                ...
                function highlightCurrentSlide() {
                  ...
                  $('li.slide-menu-item, li.slide-menu-item-vertical').each(
                    function (e) {
                      const h = $(e).data('slide-h')
                      const v = $(e).data('slide-v')
                      if (
                        h < state.indexh ||
                ...
                function createSlideMenu() {
                  if (
                    ...
                    $('.slide-menu-item, .slide-menu-item-vertical').click(
                      clicked
                    )
                ...
                if (custom) {
                  ...
                  function enableCustomLinks(panel) {
                    $(panel)
                      .find('ul.slide-menu-items li.slide-menu-item')
                  ...
                  custom.forEach(function (element, index, array) {
                    const panel = $(
                      '<div data-panel="Custom' +
                        index +
                        '" class="slide-menu-panel slide-menu-custom-panel"></div>'
                    )
                    if (element.content) {
                ...
                if (themes) {
                  var panel = $(
                    '<div data-panel="Themes" class="slide-menu-panel"></div>'
                  ).appendTo(panels)
                  var menu = $('<ul class="slide-menu-items"></ul>').appendTo(
                    panel
                  )
                  themes.forEach(function (t, i) {
                    $(
                      '<li class="slide-menu-item" data-theme="' +
                        t.theme +
                        '" data-item="' +
                        (i + 1) +
                        '">' +
                ...
                if (transitions) {
                  var panel = $(
                    '<div data-panel="Transitions" class="slide-menu-panel"></div>'
                  ).appendTo(panels)
                  var menu = $('<ul class="slide-menu-items"></ul>').appendTo(
                    panel
                  )
                  ;[
                    ...
                    $(
                      '<li class="slide-menu-item" data-transition="' +
                        name.toLowerCase() +
                        '" data-item="' +
                        (i + 1) +
                        '">' +
                ...
                if (openSlideNumber) {
                  ...
                  $(
                    '<div class="slide-number-wrapper"><a href="#"></a></div>'
                  ).insertAfter($('div.slide-number'))
                ...
                $('.slide-menu-panel .slide-menu-items li').mouseenter(
                  function (event) {
                    if (mouseSelectionEnabled) {
                      $('.active-menu-panel .slide-menu-items li').removeClass(
                        'selected'
                      )
    ...
    function scriptPath() {
      ...
      if (document.currentScript) {
        ...
        const sel = document.querySelector('script[src$="/menu.js"]')
        if (sel) {
